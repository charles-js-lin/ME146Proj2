%% Task 6(b) Problem Statement
% Modify the program for Task 4 to eliminate the PV cell and deliver the
% solar energy directly to the TE module. This means that in calculating
% errorEQ, Q_pv is instead Q_sun! Q_sun = 0.95*P"*A_pv = 0.95*rc*ID*A_pv
tic;
tol = 1e-5;
...
while abs(errorEQ(2))>tol || (errorEQ(1)*errorEQ(3)<0)
    ...
    Q_sun = 0.95*rc*ID*A_pv;%P*A_pv-PL; %Q_sun now acts as Q_pv
    Qplot(i) = Q_sun;
%% Step 3: Task 3 (TE Module Analysis)

    TH = T_pv(2) - Q_sun/A_cont;
...
syms IL_te
heatRateEqn = QH_result == n*a*IL_te*TH + A_batt*(TH-TC_result)-0.5*IL_te^2*R_batt(2);
...
legend("Q_{sun}","Q_{H,te}",'Location','southeast');
xlabel("T_{pv} [K]");
